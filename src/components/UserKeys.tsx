    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">You haven't created any Nostr keys yet.</p>
          <button
          ...
          >
                ...
                <div>
                  ...
                  {userKey.key.relays && userKey.key.relays.length > 0 && (
                    <p className="text-xs text-gray-400 mt-1">
                      {userKey.key.relays.length} relay{userKey.key.relays.length !== 1 ? 's' : ''} connected
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100">
              <div className="flex justify-between text-xs text-gray-500">
                <span>Created: {new Date(userKey.createdAt).toLocaleDateString()}</span>
                <span>Updated: {new Date(userKey.updatedAt).toLocaleDateString()}</span>
              </div>
            </div>
          </div>
        ))}
