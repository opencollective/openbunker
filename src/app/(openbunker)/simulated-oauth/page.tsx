...
export default function OpenBunkerAuthPage() {
  ...
  const [step, setStep] = useState<'initial' | 'processing' | 'complete'>('initial');
  ...
  const startDiscordOAuth = () => {
    ...
    const clientId = process.env.NEXT_PUBLIC_DISCORD_CLIENT_ID || 'your-discord-client-id';
    const redirectUri = encodeURIComponent(`${window.location.origin}/openbunker-auth`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-indigo-100 via-white to-purple-100">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">
            {window.opener ? 'Closing popup...' : 'Redirecting back to login...'}
          </p>
        </div>
      </div>
    );
